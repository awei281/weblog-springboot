package com.wlog.wlogcommon.file.core.client.s3;

import cn.hutool.core.io.IoUtil;
import cn.hutool.core.util.StrUtil;
import cn.hutool.http.HttpUtil;
import com.wlog.wlogcommon.file.AbstractFileClient;
import io.minio.*;
...
public class S3FileClient extends AbstractFileClient<S3FileClientConfig> {
    ...
    private String buildEndpointURL() {
        ...
        return StrUtil.format("http://{}", config.getEndpoint());
    }
    ...
    public String upload(byte[] content, String path, String type) throws Exception {
        ...
        String datePrefixedPath = currentDate + "/" + path;

        // 执行上传
        client.putObject(PutObjectArgs.builder()
                .bucket(config.getBucket()) // bucket 必须传递
                .contentType(type)
                .object(datePrefixedPath) // 相对路径作为 key
                .stream(new ByteArrayInputStream(content), content.length, -1) // 文件内容
                .build());
        // 拼接返回路径
        if (config.getDomain().contains(S3FileClientConfig.ENDPOINT_ALIYUN)) {
            return config.getDomain().replaceAll("-internal", "") + "/" + datePrefixedPath;
        }

        return config.getDomain() + "/" + datePrefixedPath;
    }
